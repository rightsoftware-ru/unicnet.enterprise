...
step_uninstall_full() {
  ...
  # 5. Удаляем каталог репозитория
  log "5. Удаляю каталог репозитория..."
  if [ -d "$REPO_PATH" ]; then
    rm -rf "$REPO_PATH" && log "   Каталог $REPO_PATH удален" || warn "   Не удалось удалить каталог $REPO_PATH"
  else
    info "   Каталог репозитория не существует: $REPO_PATH"
  fi
  
  # 6. Удаляем конфиг установщика и временные файлы
  log "6. Удаляю конфиг установщика и временные файлы..."
  rm -f "$SCRIPT_CWD/$CONFIG_FILE" "$REALM_JSON_TMP" 2>/dev/null && log "   Временные файлы удалены" || true
